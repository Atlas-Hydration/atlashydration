...
(function() {
  ...
  var WAVE_FRAG = [
    ...
    '  float t = u_time;',
    '',
    '  // --- Back wave (darker, sits higher, offset phase) ---',
    '  float backWave = waveSurface(uv.x, t, 3.14);',
    '  float backSurface = 0.55 + backWave;',
    '  float backEdge = smoothstep(backSurface - 0.03, backSurface + 0.015, y);',
    '',
    '  // Back wave color: slightly darker / tinted version of fill',
    '  float backCaustic = noise(vec2(uv.x * 18.0 + t * 1.0, y * 8.0 - t * 0.3));',
    '  backCaustic = smoothstep(0.35, 0.75, backCaustic) * 0.12;',
    '  vec3 backColor = u_color * 0.82 + backCaustic;',
    '',
    '  // Highlight at back wave crest',
    '  float backHL = smoothstep(backSurface + 0.015, backSurface - 0.01, y);',
    '  backHL *= smoothstep(backSurface - 0.05, backSurface, y);',
    '  backColor += backHL * 0.15;',
    '',
    '  // --- Front wave (main fill color, sits lower) ---',
    '  float frontWave = waveSurface(uv.x, t, 0.0);',
    '  float frontSurface = 0.45 + frontWave;',
    '  float frontEdge = smoothstep(frontSurface - 0.03, frontSurface + 0.015, y);',
    '',
    '  // Front wave color: the target section bg',
    '  float frontCaustic = noise(vec2(uv.x * 22.0 + t * 1.3, y * 10.0 - t * 0.5));',
    '  frontCaustic = smoothstep(0.4, 0.8, frontCaustic) * 0.1;',
    '  vec3 frontColor = u_color * (0.95 + frontCaustic);',
    '',
    '  // Highlight at front wave crest',
    '  float frontHL = smoothstep(frontSurface + 0.015, frontSurface - 0.01, y);',
    '  frontHL *= smoothstep(frontSurface - 0.05, frontSurface, y);',
    '  frontColor += frontHL * vec3(0.10, 0.12, 0.14);',
    '',
    '  // --- Composite: back wave behind, front wave on top ---',
    '  // Start fully transparent',
    '  vec4 result = vec4(0.0);',
    '',
    '  // Layer back wave',
    '  result = mix(result, vec4(backColor, 1.0), backEdge);',
    '',
    '  // Layer front wave on top',
    '  result = mix(result, vec4(frontColor, 1.0), frontEdge);',
    '',
    '  gl_FragColor = result;',
    '}'
  ].join('\n');
